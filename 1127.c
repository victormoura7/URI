...
char KMP(int distanciasMusica[300001], int distanciasTrecho[300001], int n, int m, int tabelaPrefixos[30001]) {
    int q = 0;
    int i = 1;
    for (; i<= n; i++) {
...
int *computaPrefixo(int distanciasTrecho[300001], int m) {
...
int *calculaDistancia(int vetor[300001], int tamanho) {
...
int main(int argc, char const *argv[]) {
    ...
    while (1) {
        ...
        notasMusicaFinal = insereVetor(notasMusica);
        notasTrechoFinal = insereVetor(notasTrecho);

        distanciasMusica = calculaDistancia(notasMusicaFinal, tamanhoMusica);
        distanciasTrecho = calculaDistancia(notasTrechoFinal, tamanhoTrecho);
        tabelaPrefixos = computaPrefixo(distanciasTrecho, tamanhoTrecho-1);
        char resultado = KMP(distanciasMusica, distanciasTrecho, tamanhoMusica-1, tamanhoTrecho-1, tabelaPrefixos);
        printf("%c\n", resultado);  
        free(tabelaPrefixos);
